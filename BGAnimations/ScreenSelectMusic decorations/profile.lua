...
t[#t+1] = LoadFont("_wendy small")..{
	InitCommand=function(self)
		self:xy(frameX+5,frameY+offsetY-9):zoom(0.5):halign(0):diffuse(getMainColor('positive')):settext("Profile Info (WIP)")
	end
}
...
local profilebuttons = Def.ActorFrame{
	...
	LoadFont("_wendy small") .. {
		InitCommand=function(self)
			self:x(130):diffuse(getMainColor('positive')):settext("Import Scores"):zoom(0.3)
		end
	},
	Def.Quad{
		InitCommand=function(self)
			self:x(105):zoomto(100,20):diffusealpha(buttondiffuse)
		end,
		MouseLeftClickMessageCommand=function(self)
			if ButtonActive(self) and rankingSkillset == 1 then
	...
	LoadFont("_wendy small") .. {
		InitCommand=function(self)
			self:x(260):diffuse(getMainColor('positive')):settext("Validate All"):zoom(0.3)
		end
	},
	Def.Quad{
		InitCommand=function(self)
			self:x(210):zoomto(100,20):diffusealpha(buttondiffuse)
		end,
		MouseLeftClickMessageCommand=function(self)
			if ButtonActive(self) and rankingSkillset == 1 then
