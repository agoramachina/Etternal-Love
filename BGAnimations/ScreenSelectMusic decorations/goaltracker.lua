...
t[#t+1] = LoadFont("_wendy small")..{
	InitCommand=function(self)
		self:xy(frameX+5,frameY+offsetY-9):zoom(0.5):halign(0):diffuse(getMainColor('positive')):settext("Goal Tracker (WIP)")
	end
}
...
local function makescoregoal(i)
	...
	local t = Def.ActorFrame{
		...
		Def.ActorFrame{
			...
			Def.Quad{
				InitCommand=function(self)
					self:xy(-20,20):zoomto(frameWidth-20,goalYspacing-2):halign(0):valign(1):diffuse(getMainColor('frames')):diffusealpha(0.35)
				end
			},
...
local function filterButton(i)
	local t = Def.ActorFrame{
		Def.Quad{
		InitCommand=function(self)
			self:xy(13+frameX+rankingX+(i-1+i*(1/(1+3)))*rankingTitleWidth,frameY+rankingY-60):zoomto(rankingTitleWidth+22,32):halign(0.5):valign(0):diffuse(getMainColor('frames')):diffusealpha(0.35)
		end,
		SetCommand=function(self)
		...
		LoadFont("_wendy small") .. {
			InitCommand=function(self)
				self:xy(17+frameX+rankingX+(i-1+i*(1/(1+3)))*rankingTitleWidth,frameY+rankingY-45):halign(0.5):diffuse(getMainColor('positive')):maxwidth(rankingTitleWidth):maxheight(25)
			end,
			BeginCommand=function(self)
				self:settext(fawa[i])
			end,
...
r[#r+1] = Def.ActorFrame{
	InitCommand=function(self)
		self:xy(frameX+25,frameY+rankingY+255)
	end,
	Def.Quad{
		InitCommand=function(self)
			self:xy(300,-8):zoom(40,20):halign(0):valign(0):diffuse(getMainColor('frames')):diffusealpha(buttondiffuse)
		end,
		MouseLeftClickMessageCommand=function(self)
